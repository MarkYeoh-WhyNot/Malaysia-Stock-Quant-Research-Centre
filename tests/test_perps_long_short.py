...
def test_crypto_funding_drag_reduces_long_pnl(monkeypatch):
    import agents.backtest_engineer.engine as be_mod
    ...
    perf_funded = engine._compute_performance(be, df, sig, "1d")
...
def test_leverage_scales_returns_and_liquidation_caps_losses(monkeypatch):
    ...
    sig = pd.Series(1.0, index=df.index)
    perf_1x = engine._compute_performance(be, df, sig, "1d", leverage=1.0)
    perf_5x = engine._compute_performance(be, df, sig, "1d", leverage=5.0)
...
def test_leverage_capped_at_max_leverage(monkeypatch):
    ...
    df = _price_df()
    sig = pd.Series(1.0, index=df.index)
    perf = engine._compute_performance(be, df, sig, "1d", leverage=10.0)  # request > cap
